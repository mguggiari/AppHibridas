...
function createProductListPage(products) {
    ...
    for(let i = 0; i < products.length; i++){
        html += `<li>${products[i].name} <a href="/products/${products[i]._id}">Ver detalle</a><a href="/products/${products[i].id}/edit">Editar</a><a href="/products/${products[i]._id}/delete">Eliminar</a></li>`;
    }
    html += '</ul>';
...
function createProductEditFormPage(product) {
    ...
    html += 
        `<form action="/products/${product._id}/edit" method="POST"> //aca le estas devolviendo el id del producto que se esta editando
