...
function editProductForm(req, res) {
    ...
    service.getProductById(id)
        .then(function (product){
            if(product){
                res.send(view.createProductEditFormPage(product))
            }
            else{
                res.send(view.createPage('Ups', '<h1>No se encontro el producto solicitado</h1>'))
            }
        })
}   
...
function editProduct(req, res) {
    ...
    //aca lo que llega por post es lo que se va a modificar
    const product = {
        name: req.body.name, 
        description: req.body.description,
        price: req.body.price
    ...
    service.editProduct(id, product)
        .then(function (product){
            if(product){
                res.send(view.createPage('Producto editado', `<p>Producto ${product.name} editado con exito ${product.id} </p>`))
            }
            else{
                res.send(view.createPage('Ups', '<h1>No se encontro el producto solicitado</h1>'))
            }
...
function deleteProduct(req, res) {
    ...
    service.deleteProduct(id)
        .then(function (product){
            if(product){
                res.send(view.createPage('Producto eliminado', `<p>Producto ${product.name} eliminado con exito</p>`))
            }
            else{
                res.send(view.createPage('Ups', '<h1>No se encontro el producto solicitado</h1>'))
            }
